...
function BookConditionReview({ notice, onSaved, onError }) {
  ...
  return (
    <form
      onSubmit={submit}
      className="mt-4 grid gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-950/20 sm:grid-cols-2 lg:grid-cols-4"
    >
      <label className="text-xs font-semibold text-gray-600 dark:text-gray-300">
        ...
        <select
          ...
          className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-[#393954] dark:bg-[#10101d]"
        >
      ...
      <label className="text-xs font-semibold text-gray-600 dark:text-gray-300">
        ...
        <select
          value={status}
          disabled={notice.current_status === 'ISSUED'}
          onChange={(event) => setStatus(event.target.value)}
          className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm disabled:opacity-60 dark:border-[#393954] dark:bg-[#10101d]"
        >
          {['AVAILABLE', 'ISSUED', 'DAMAGED', 'LOST', 'RESERVED'].map((value) => (
            <option key={value}>{value}</option>
          ))}
        </select>
      </label>
      <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 sm:col-span-2 lg:col-span-1">
        Review notes
        <input
          value={notes}
      ...
      <div className="flex items-end">
        <button
          disabled={saving}
          className="w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-60"
        >
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="Approvals, deposit activity, edited values, and annual book-condition reminders."
      />

      {message && (
        <div className="rounded-xl border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/30 p-3 text-sm">
          {message}
        </div>
      )}
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-3 text-sm font-bold whitespace-nowrap border-b-2 flex items-center gap-1.5 ${
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-[#292944] bg-white dark:bg-[#17172a] overflow-hidden">
        {visibleNotifications.length === 0 ? (
          <EmptyState
            ...
            description="No notifications match the selected filters."
          />
        ) : (
          visibleNotifications.map((notice) => (
            <div
              key={`${notice.action}-${notice.audit_id || notice.record_id}`}
              className="p-5 border-b last:border-b-0 border-gray-200 dark:border-[#292944]"
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                ...
                <div className="flex gap-2 shrink-0">
                  {notice.requires_approval && notice.action === 'DEPOSIT_CORRECTION_REQUEST' && (
                    <>
                      <button
                        onClick={() => decideDepositCorrection(notice, 'approve')}
                        className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-bold"
                      >
                        Approve Correction
                      </button>
                      <button
                        onClick={() => decideDepositCorrection(notice, 'reject')}
                        className="px-3 py-1.5 rounded-lg bg-gray-200 dark:bg-[#292944] text-xs font-bold"
                      >
                        Reject
                      </button>
                    </>
